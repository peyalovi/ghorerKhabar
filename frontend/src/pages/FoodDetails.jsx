...
const FoodDetails = () => {
    const { foods, navigate, currency, addToCart, removeFromCart, cartItems, isChef } = useAppContext();
    const { id } = useParams();
    ...
    useEffect(() => {
        if (foods.length > 0 && food) {
            let foodsCopy = foods.filter((item) => food.category === item.category);
                ...
                <div className="text-sm w-full md:w-1/2">
                    ...
                    <div className="mt-6">
                        <p className="text-gray-500/70 line-through">
                            MRP: {food.price}{currency}
                        </p>
                        <p className="text-2xl font-medium">
                            MRP: {food.offerPrice}{currency}
                        </p>
                        ...
                        <div className="flex items-center mt-10 gap-4 text-base">
                            ...
                            {!cartItems[food._id] ? (
                                <button
                                    ...
                                    className="flex-1 py-3.5 cursor-pointer font-medium bg-gray-100 text-gray-800/80 hover:bg-gray-200 transition"
                                >
                                    Add to Cart
                                </button>
                            ) : (
                                <div className="flex items-center justify-between flex-1 bg-primary/25 rounded select-none">
                                    <button
                                        onClick={() => removeFromCart(food._id)}
                                        className="cursor-pointer text-lg font-bold px-4 py-3"
                                    >
                                        -
                                    </button>
                                    <span className="w-6 text-center">{cartItems[food._id]}</span>
                                    <button
                                        onClick={() => addToCart(food._id)}
                                        className="cursor-pointer text-lg font-bold px-4 py-3"
                                    >
                            ...
                            <button
                                ...
                                className="flex-1 py-3.5 cursor-pointer font-medium bg-primary text-white hover:bg-primary-dull transition"
                            >
                                Order now
                            </button>
                        </div>
